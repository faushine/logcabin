 ...
 */
            struct FileNotifier : public Event::File {
                FileNotifier(Event::Loop &eventLoop, int fd, Ownership ownership)
                        : Event::File(fd, ownership), eventLoop(eventLoop), count(0) {
                }
 ...
 */
            struct TimerNotifier : public Event::Timer {
                explicit TimerNotifier(Event::Loop &eventLoop)
                        : Event::Timer(), eventLoop(eventLoop) {
                }
...
////////// ClientSession::Response //////////
        ...
        ClientSession::Response::Response()
                : status(Response::WAITING), reply(), hasWaiter(false), ready() {
        }

////////// ClientSession::Timer //////////

        ClientSession::Timer::Timer(ClientSession &session)
                : Event::Timer(), session(session) {
        }
...
////////// ClientSession //////////
        ...
        ClientSession::ClientSession(Event::Loop &eventLoop,
                ...
                , PING_TIMEOUT_NS(config.read<uint64_t>(
                        "tcpHeartbeatTimeoutMilliseconds", 500) * 1000 * 1000), eventLoop(eventLoop), address(address),
                  messageSocketHandler(*this), timer(*this), mutex(), nextMessageId(0), responses(), errorMessage(),
                  numActiveRPCs(0), activePing(false), messageSocket(), timerMonitor(eventLoop, timer) {
